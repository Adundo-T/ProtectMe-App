  ...
  const actions: AppContextValue['actions'] = {
    ...
    syncNow: async () => {
      ...
      try {
        ...
        const pendingIds = state.reports.filter((report) => report.status !== 'synced').map((report) => report.id);
        await Promise.all(pendingIds.map((id) => updateReportStatus(id, 'synced')));
        await refreshReports();
